...
3. Invoke using: `Skill` tool with `connectors-python:connector-{type}` format

**Examples:**
- Neon database → invoke `connectors-python:connector-neon` skill first
- PostgreSQL → invoke `connectors-python:connector-postgresql` skill first
- OpenAI → invoke `connectors-python:connector-openai` skill first
...
**Frontend work**: When implementing or modifying frontend code (HTML, CSS, UI, static assets), also invoke the `frontend-design:frontend-design` skill using the Skill tool and follow its guidelines for design and UX.
